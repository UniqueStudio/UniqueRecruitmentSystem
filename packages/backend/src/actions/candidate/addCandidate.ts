...
export const addCandidate: RequestHandler = async (req, res, next) => {
    try {
        ...
        if (req.file) {
            ...
            await moveFile(oldPath, filepath, `${name} - ${filename}`);
        }
        await CandidateRepo.createAndInsert({
            name,
            gender,
            grade,
            institute,
...
export const addCandidateVerify = [
    ...
    body('grade').isInt({ lt: 7, gt: -1 }).withMessage('Grade is invalid!'),
    body('institute').isString().withMessage('Institute is invalid!'),
    body('major').isString().withMessage('Major is invalid!'),
    body('gender').isInt({ lt: 3, gt: -1 }).withMessage('Gender is invalid!'),
    ...
    body('rank').isInt({ lt: 5, gt: -1 }).withMessage('Rank is invalid!'),
